...
| C3 Loop Git/Sync 增强 | in_progress | 正在增加 git pull/commit/push/sync recovery 生命周期 |
...
| C3.2 实现 git lifecycle runner MVP | in_progress | 已为 `run-loop` 引入 clean check、pull/push 门禁骨架，正在收尾 sync 恢复策略 | `awf run-loop` smoke |
