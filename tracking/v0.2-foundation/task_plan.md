...
| C2 Planner 增强 | in_progress | 正在让需求拆解从固定 5-case 升级为按复杂度和项目能力生成 |
| C3 Loop Git/Sync 增强 | pending | 增加 git pull/commit/push/sync recovery 生命周期 |
...
| C2.2 实现智能 case 生成 MVP | in_progress | 已让 `plan` 生成可变数量和不同类型的 case，正在收尾文档与示例 | `awf plan` smoke |
| C3.1 定义 loop state / sync schema | pending | 扩充 git/sync 恢复和 case 生命周期状态 | schema 完整 |
| C3.2 实现 git lifecycle runner MVP | pending | 为 `run-loop` 引入 pull/status/commit/push 门禁骨架 | `awf run-loop` smoke |
